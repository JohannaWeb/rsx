...
// GPUSTAT bits that we model directly.
const GPU_STATUS_INTERLACE_FIELD_BIT: u32 = 1 << 13;
const GPU_STATUS_DISPLAY_DISABLED_BIT: u32 = 1 << 23;
const GPU_STATUS_IRQ_REQUEST_BIT: u32 = 1 << 24;
const GPU_STATUS_DMA_REQUEST_BIT: u32 = 1 << 25;
const GPU_STATUS_READY_TO_RECEIVE_CMD_BIT: u32 = 1 << 26;
const GPU_STATUS_READY_TO_RECEIVE_DMA_BIT: u32 = 1 << 28;
const GPU_STATUS_DMA_DIRECTION_SHIFT: u32 = 29;
const GPU_STATUS_DMA_DIRECTION_MASK: u32 = 0x3;
const GPU_COMMAND_FIFO_LIMIT_WORDS: usize = 16;
const DEFAULT_DISPLAY_WIDTH: u16 = 320;
const DEFAULT_DISPLAY_HEIGHT: u16 = 240;
...
pub struct GpuDebugState {
    ...
    pub gp0_fifo_depth: usize,
    pub image_load_active: bool,
    pub status: u32,
}
...
impl Gpu {
    ...
    pub fn write_gp0(&mut self, word: u32) {
        ...
        let words = std::mem::take(&mut self.gp0_words);

        if self.config.trace_gpu {
            log::info!(
                "GPU GP0 command {:#02x} words={:?}",
                (words[0] >> 24) as u8,
                words
            );
        }
    ...
    pub fn write_gp1(&mut self, word: u32) {
        match (word >> 24) as u8 {
            ...
            0x04 => self.dma_direction = (word & GPU_STATUS_DMA_DIRECTION_MASK) as u8,
            0x05 => {
    ...
    pub fn copy_display_into(&self, out: &mut [u8]) {
        ...
        for (index, chunk) in out[..needed].chunks_exact_mut(3).enumerate() {
            let x = index % w;
            let y = index / w;
    ...
    /// Returns the full 1024x512 VRAM contents as packed RGB24 bytes.
    pub fn vram_rgb(&self) -> Vec<u8> {
        let mut rgb = Vec::with_capacity(VRAM_WIDTH * VRAM_HEIGHT * 3);
        for pixel in self.vram.iter().copied() {
            rgb.extend_from_slice(&rgb555_to_rgb888(pixel));
        }
        rgb
    }
    ...
    pub fn debug_state(&self) -> GpuDebugState {
        GpuDebugState {
            ...
            gp0_fifo_depth: self.gp0_words.len(),
            image_load_active: self.image_load.is_some(),
    ...
    fn status_word(&self) -> u32 {
        let mut status = GPU_STATUS_INTERLACE_FIELD_BIT
            | GPU_STATUS_READY_TO_RECEIVE_CMD_BIT
            | GPU_STATUS_READY_TO_RECEIVE_DMA_BIT;

        if !self.display_enabled {
            status |= GPU_STATUS_DISPLAY_DISABLED_BIT;
        }
        if self.irq_requested {
            status |= GPU_STATUS_IRQ_REQUEST_BIT;
        }

        match self.dma_direction {
            0 => {}
            1 => {
                if self.gp0_words.len() < GPU_COMMAND_FIFO_LIMIT_WORDS {
                    status |= GPU_STATUS_DMA_REQUEST_BIT;
                }
            }
            2 => {
                if self.image_load.is_none() {
                    status |= GPU_STATUS_DMA_REQUEST_BIT;
                }
            }
            3 => {}
        ...
        status | ((self.dma_direction as u32) << GPU_STATUS_DMA_DIRECTION_SHIFT)
    }
    ...
    fn execute_gp0(&mut self, command: Gp0Command, words: &[u32]) {
        ...
        match command {
            ...
            Gp0Command::MonoTriangle => {
                ...
                self.draw_triangle(
                    color,
                    [vertex(words[1]), vertex(words[2]), vertex(words[3])],
                );
            }
    ...
    fn draw_triangle(&mut self, color: u16, points: [(i32, i32); 3]) {
        ...
        let min_x = pts
            ...
            .min()
            .unwrap()
            .max(self.drawing_area.left);
        let max_x = pts
            .iter()
            .map(|(x, _)| *x)
            .max()
            .unwrap()
            .min(self.drawing_area.right);
        let min_y = pts
            .iter()
            .map(|(_, y)| *y)
            .min()
            .unwrap()
            .max(self.drawing_area.top);
        let max_y = pts
            .iter()
            .map(|(_, y)| *y)
            .max()
            .unwrap()
            .min(self.drawing_area.bottom);
...
mod tests {
    ...
    fn fills_vram_rectangle() {
        ...
        let rgb = gpu.vram_rgb();
        let offset = (20 * VRAM_WIDTH + 10) * 3;
    ...
    fn uploads_cpu_image_to_vram() {
        ...
        let rgb = gpu.vram_rgb();
        let first = (5 * VRAM_WIDTH + 4) * 3;
        let second = (5 * VRAM_WIDTH + 5) * 3;
